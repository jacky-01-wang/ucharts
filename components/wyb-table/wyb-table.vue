			...
			<view class="wyb-table-header" :style="{borderBottom: '1px solid' + borderColor}">
				...
				<view ref="iosBug" class="wyb-table-header-item" v-for="(item, index) in headers" :key="item.key" @tap="onHeaderItemTap(index)"
				 :style="{
					 ...
					 <view class="sanjiaoContainer wyb-table-header-icon"  v-if="index!==0">
					 	<text class="topSanjiao" :style="{
							color: sortWays[sortWay] === 'asc' && sortActiveKey === item.key ? 
								headerFtColor : RGBChange(headerFtColor, 0.7, 'light'),
							fontWeight: 'normal',
							marginBottom: '-12px',
							transform: 'scale(0.4)'}"  @click="topSort" data-type="top" :data-index="index">
					 		
					 	</text> 
					 	<text class="bottomSanjiao" :style="{
							color: sortWays[sortWay] === 'inv' && sortActiveKey === item.key ? 
								headerFtColor : RGBChange(headerFtColor, 0.7, 'light'),
							fontWeight: 'normal',
							transform: 'scale(0.4)'}" @click="bottomSort" data-type="bottom" :data-index="index">
					 	</text>
					 </view>
...
<style>
	...
	.topSanjiao{
		...
		width: 0;
		height: 0;
		border-left: 25upx solid transparent;  /*左边框实现透明;25pupx是三角形左边框的宽度*/
		border-right: 25upx solid transparent;
		border-bottom: 30upx solid #000;
